...
def _route_car_control(raw_utterance: str, normalized: str, language: str) -> dict:
    ...
    # --- START MODIFICATION ---
    from services.car_controller import has_temp_set_cue, parse_hvac_temp_celsius
    ...
    # Wave1: out-of-range HVAC set-point → soft reject (no actuator write)
    if has_temp_set_cue(normalized) and parse_hvac_temp_celsius(normalized) is None:
        if language == "en":
            answer = "Temperature must be within 16–30°C or 60–85°F."
        else:
            answer = "Nhiệt độ ngoài khoảng cho phép (16–30°C / 60–85°F)."
        return {
            "query": raw_utterance,
            "answer": answer,
            "command_id": None,
...
async def route_text_query(
    ...
    x_skip_tts: str | None = Header(
        default=None,
        alias="X-Skip-TTS",
        description="Set to `1` to skip gateway TTS (cockpit local speakOut).",
    ),
):
    # --- START MODIFICATION ---
    ...
    try:
        ...
        # MODIFIED: Wave2 — dropped per-query Core health probe (latency SLO)
        ...
        # --- START MODIFICATION ---
        # Wave2: X-Skip-TTS → text-only; cockpit speaks via speakOut / /copilot/tts
        skip_tts = str(x_skip_tts or "").strip() == "1"
        if skip_tts:
            audio_bytes, tts_ms = None, 0
        else:
            audio_bytes, tts_ms = await synthesize_speech_bytes(
                answer_text, language=language
            )
